...
class CaptchaNoiseFilter:
    ...
    def __call__(self, img):
        img = img.convert('L')
        # Median filter is excellent for removing line noise
...
def greedy_decoder(logits):
    """Decodes the CTC logit output into a string for accuracy comparison"""
    # logits shape: [SeqLen, Batch, Classes]
    pred_indices = logits.argmax(2) # [SeqLen, Batch]
    pred_indices = pred_indices.transpose(0, 1).cpu().numpy() # [Batch, SeqLen]
...
def calculate_metrics(predictions, targets):
    """Calculates Sequence (Word) Accuracy and Character Accuracy"""
    correct_words = 0
    correct_chars = 0
    total_chars = 0
...
[Image of deep ResNet CNN backbone feeding into Bidirectional LSTM]
class CRNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.cnn = nn.Sequential(
            ...
            nn.Dropout(0.2) # Prevent overfitting on small dataset
        )
...
def train():
    ...
    # Cosine Annealing bounces the LR to escape plateaus
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Show a sample prediction every 10 epochs
        if epoch % 10 == 0 or val_w_acc > 5.0:
            print(f"    * Sample - True: '{val_targets[0]}' | Pred: '{val_preds[0]}'")

        # Save Best Model strictly based on Word Accuracy
