...
EPOCHS = 50
LEARNING_RATE = 0.001
DATA_CSV = 'data_clean.csv' # Use the sanitized file
IMG_DIR = 'captchas/'
...
def setup_dataset():
    if os.path.exists('dataset/train') and os.path.exists('dataset/test'):
        if len(os.listdir('dataset/train')) > 1:
            print(">>> Using existing dataset in ~/mlPro/dataset.")
...
class CaptchaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # KEYERROR FIX: Only map chars that exist in CHAR_MAP
        label_indices = [CHAR_MAP[c] for c in label_str if c in CHAR_MAP]
        if not label_indices: label_indices = [1] # Fallback for empty/invalid labels
...
def train():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"Epoch {epoch} | Train: {avg_train:.4f} | Test: {avg_test:.4f}")
